...
function searchCallback(results) {
    ...
    for(i = 0; i <= 8; i++){
    	...
    	$("#returnGame").append("<div class='container well col-md-4 col-xs-12' id='mainWindow'><div class='hidden-sm hidden-xs' id='gameImage'><img src='" + picture + "'/></div><div class='outer' id='gameTitle'><p class='lead text-center'>" + results[i].name + "</p></div><p id='textHidden'>" + results[i].deck + "<br><a href='" + game + "' target='_blank'>Take me there</a></p><button class='btn-sm btn-primary' id='removeGame'>Remove Game</button></div>").hide().fadeIn(500);
	}
}
